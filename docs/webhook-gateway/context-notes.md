...
환율 반올림한 값이다. "10만 건당"은 표기일 뿐이고 계량은 건 단위(0.04원/건)다. Hookdeck·Svix도
그렇게 한다. 10만 건 단위 올림은 1건 초과에 4,000원을 물려 설명이 안 되고, 내림은 9만 9,999건까지
무료가 돼 종량의 의미가 없다. 원 단위는 내림한다. 토스 승인 금액이 정수여야 하고, 사용량 스냅샷에서
세운 "청구는 고객에게 유리한 쪽으로 틀린다"는 원칙과 같다.
포함량은 Hookdeck처럼 전 플랜 월 1만으로 같다. 유료 플랜의 차이는 포함량이
