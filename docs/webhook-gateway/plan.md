...
- **전달 워커.** Valkey Stream 컨슈머 그룹. 목적지로 HTTP POST(원본 헤더 일부 + 본문 그대로 +
  `X-Relaydam-*` 메타 헤더). 타임아웃, 지수 백오프 + 지터, 최대 시도, 시도마다 기록.
...
9. **at-least-once.** 워커 처리는 최소 1회. 목적지 측 멱등은 `X-Relaydam-Event-Id` 헤더로 지원.
...
11. **과금.** `organization.plan`이 적용 플랜이고 `subscription`은 결제 상태다. 플랜별 값은 코드
    ...
    | 초과 | 429 거부 | 0.25원/건 | 0.25원/건 | 0.25원/건 |
    | 멤버 | 1명 | 1명 | 10명 | 무제한 |
    | 보존 | 3일 | 7일 | 30일 | 90일 |
